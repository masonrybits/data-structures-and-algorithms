...
const count = (target, input) => {
  return input.reduce((accumulator, numberArr) => {
    return accumulator + numberArr.reduce((accumulator, number) => {
      if (number === target) {
...
let findMaleAndFemale = (data) => {
  let answer = data.filter(person => person.gender === 'male' || person.gender === 'female').map(person => person.name);
...
let findShortest = (data) => {
  return data.reduce((accumulator, person) => {
    if (person.name.length > accumulator.length) {
      return accumulator;
    } else {
      return person.name
    }
